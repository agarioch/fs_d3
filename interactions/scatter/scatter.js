async function drawScatter() {
  ...
  const transitionTooltip = d3
    .transition()
    .duration(100)
    .ease(d3.easeCubicInOut);

  function onMouseEnter(data, index) {
    const formatHumidity = d3.format(".2f");
    tooltip.select("#humidity").text(formatHumidity(yAccessor(data)));

    const formatDewPoint = d3.format(".2f");
    tooltip.select("#dew-point").text(formatDewPoint(xAccessor(data)));

    const dateParser = d3.timeParse("%Y-%m-%d");
    const formatDate = d3.timeFormat("%B %A %-d, %Y");
    tooltip.select("#date").text(formatDate(dateParser(data.date)));

    const x = xScale(xAccessor(data)) + dimensions.margin.left;
    const y = yScale(yAccessor(data)) + dimensions.margin.top;
