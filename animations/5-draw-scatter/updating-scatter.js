async function drawChart() {
  ...
  dataset = dataset
    .sort((a, b) => dateAccessor(a) - dateAccessor(b))
    .slice(0, 10);
  ...
  const width = d3.min([window.innerWidth * 0.95, window.innerHeight * 0.95]);
  let dimensions = {
    width: width,
    height: width,
    margin: {
  ...
  function drawScatter(dataset) {
    ...
    const newDots = dots.enter().append("circle").attr("r", 0);
